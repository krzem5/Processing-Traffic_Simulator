class Road {
  ...
  JSONObject conns;
  RLightObject LIGHTS;
  ArrayList<RWeightPoint> weight_points=null;
  ...
  Road(Engine ENGINE) {
    ...
    this.j_st=null;
    this.j_ed=null;
    this.LIGHTS=new RLightObject(this.ENGINE, this);
    this.update_lines();
  }
  ...
  void update(boolean ex) {
    if (this.ENGINE.EDITING_ROAD_INPUT==true&&this.weight_points!=null) {
      for (RWeightPoint p : this.weight_points) {
        p.update();
      }
    }
  ...
  void update_lines() {
    List<RLine> LINES=this.ENGINE.RMAP.get_lines(this.ID);
    if (LINES.size()==0) {
      return;
    }
    PVector aa, ab, ba, bb;
  ...
  void update_I() { 
    this.weight_points=new ArrayList<RWeightPoint>();
    if (this.is_I()==true) {
      if (this.get_I_point()==this.start) {
        for (int i=0; i<this.lanesA; i++) {
          RLine l=this.ENGINE.RMAP.get_lines(this.ID).get(this.ENGINE.MAX_LANES-1-i);
          this.weight_points.add(new RWeightPoint(this.ENGINE, l.s.copy(), l, this));
        }
      } else {
        for (int i=0; i<this.lanesB; i++) {
          RLine l=this.ENGINE.RMAP.get_lines(this.ID).get(this.ENGINE.MAX_LANES+i);
          this.weight_points.add(new RWeightPoint(this.ENGINE, l.e.copy(), l, this));
        }
      }
    }
  }
  ...
  void draw_roadmap() {
    ...
    noStroke();
    List<RLine> LINES=this.ENGINE.RMAP.get_lines(this.ID);
    RLine l=LINES.get(this.ENGINE.MAX_LANES-(this.lanesA-1)-1);
  ...
  void fromJSON(JSONObject json) {
    ...
    this.LIGHTS=new RLightObject(this.ENGINE, this);
    this.LIGHTS.fromJSON(json.getJSONObject("lights"));
  }
  JSONObject toJSON() {
    ...
    json.setJSONObject("from", start);
    json.setJSONObject("to", end);
    json.setInt("lanesA", this.lanesA);
    json.setInt("lanesB", this.lanesB);
    json.setInt("id", this.ID);
    json.setJSONObject("connections", conns);
    json.setJSONObject("lights", this.LIGHTS.toJSON());
    json.setString("type", "road");
    return json;
  }
}
