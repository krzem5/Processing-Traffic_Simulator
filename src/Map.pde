class Map {
  ...
  ArrayList<CPathPoint> get_path(RLine cell) {
    ArrayList<CPathPoint> path=new ArrayList<CPathPoint>();
    ...
    while (true) {
      ...
      next=null;
      change=false;
      if (((cell.sl==null&&cell.Asl.size()>0)||cell.sl!=null)&&!chk_ID(visited, cell.sl, cell.Asl)) {
        if (cell.sl==null) {
        ...
        if (path.size()<2) {
          ...
          path.add(new CPathPoint(this.ENGINE, v));
        }
        if (change==true) {
          path.add(new CPathPoint(this.ENGINE, cell.e.copy()));
          path.add(new CPathPoint(this.ENGINE, next.s.copy(), cell.s));
        } else {
          if (next!=null) {
            path.add(new CPathPoint(this.ENGINE, cell.e.copy()));
            path.add(new CPathPoint(this.ENGINE, cell.s.copy()));
            path.add(new CPathPoint(this.ENGINE, next.s.copy()));
          } else {
        ...
        if (((cell.el==null&&cell.Ael.size()>0)||cell.el!=null)&&!chk_ID(visited, cell.el, cell.Ael)) {
          if (cell.el==null) {
          ...
          if (change==true) {
            path.add(new CPathPoint(this.ENGINE, cell.s.copy()));
            path.add(new CPathPoint(this.ENGINE, next.s.copy(), cell.e));
          } else {
            if (next!=null) {
              path.add(new CPathPoint(this.ENGINE, cell.s.copy()));
              path.add(new CPathPoint(this.ENGINE, cell.e.copy()));
  ...
  ArrayList<CPathPoint> remove_duplicate(ArrayList<CPathPoint> path){
    ArrayList<CPathPoint> r_path=new ArrayList<CPathPoint>();
    CPathPoint l=null;
    for (CPathPoint p : path) {
  ...
  ArrayList<CPathPoint> split(ArrayList<CPathPoint> path) {
    ArrayList<CPathPoint> f_path=new ArrayList<CPathPoint>();
    ...
    for (int i=1; i<path.size(); i++) {
      ...
      RLine l=path.get(i).lane;
      ArrayList<PVector> d=this.divide(a, b);
      int j=0;
      for (PVector v : d) {
        f_path.add(new CPathPoint(this.ENGINE, v, l, (j==d.size()-1)));
        j++;
      }
    }
    return f_path;
  ...
  boolean chk_ID(IntList vs, RLine c, ArrayList<RLine> cl) {
