class Engine{
	...
	ArrayList<Car> CARS;
	boolean ROADMAP_VIEW=false;
	boolean DRAW_ARROWS=true;
	boolean LANE_LINES=false;
	...
	RoadWeightPoint updating_rpoint=null;
	Light updating_light=null;
	RoadLightObject updating_r_light;
	int LANE_WIDTH;
	int CAR_HEIGHT;
	...
	RoadLine get_car_road(){
		ArrayList<RoadLine> d=new ArrayList<RoadLine>();
		for (int i=0;i<this.road_weights.size();i++){
			...
			for (int j=0;j<l.size();j++){
				...
				RoadLine ln=this.RMAP.LINES.get(i).get(j);
	...
	void save_json(){
		...
		for (i=0;i<this.ROADS.size();i++){
			data.setJSONObject(i,this.ROADS.get(i).to_json());
		}
		for (i=0;i<this.JUNCTIONS.size();i++){
			data.setJSONObject(this.ROADS.size()+i,this.JUNCTIONS.get(i).to_json());
		}
		o.setJSONArray("board",data);
		o.setJSONObject("conns",conns);
		saveJSONObject(o,"data/"+this.BOARD_NAME);
	}
